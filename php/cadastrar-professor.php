	...
	//Saber se os campos estao vazios
		...
		if($nome != "" && $data_nascimento != "" && $identificacao != "" && $grau_academico != "" && $rua != "" && $bairro != "" && $municipio != "" && $cidade != "" && $provincia != "" && $telefone1 != ""){
			...
			if ($registro >= 1) {
					...
					header('Location: ../pages/dir-academico/cadastrar-professor.php?erro=1');
					...
					if(mysqli_query($conexao, $inserir)){
						...
						header('Location: ../pages/dir-academico/cadastrar-professor.php?sucesso=1');
					} else{
			...
			header('Location: ../pages/dir-academico/cadastrar-professor.php?valor=1');
		}
...
//Pegar os Dados
	...
	//Telefones - Tabelas diferentes
		...
		$e = $_POST['secondaryTele'];

	//Saber se os campos estao vazios
		if($nome != "" && $data_nascimento != "" && $identificacao != "" && $grau_academico != "" && $rua != "" && $bairro != "" && $municipio != "" && $cidade != "" && $primaryTele != ""){

			$sql = mysqli_query($conexao, "SELECT nome FROM professor WHERE nome = '$nome' OR bi = '$identificacao'");
			$registro = mysqli_num_rows($sql);
			...
			if ($registro >= 1) {
					...
					header('Location: ../pages/dir-academico/cadastro-professor.php?erro=1');

			} else{
				
					$inserir = "INSERT INTO professor(nome, dataNasc, grauAcademico, bi, rua, bairro, municipio, cidade) VALUES ('$nome', '$data_nascimento', '$grau_academico', '$identificacao', '$rua', '$bairro', '$municipio', '$cidade')";
					if(mysqli_query($conexao, $inserir)){


						$sql = mysqli_query($conexao, "SELECT idProfessor FROM professor WHERE nome = '$nome'");

							while ($line = mysqli_fetch_array($sql)) {
								$teste = $line['idProfessor'];
							}

							//Inserir apenas no primeiro Contacto
							if($workTele != "" && $secondaryTele != ""){

								$sql = "INSERT INTO telefone(telefone1, telefone2, telefone3, idProfessor) VALUES ('$primaryTele', '$workTele', '$secondaryTele', '$teste')";
								mysqli_query($conexao,$sql);
							} elseif ($workTele != "" && $secondaryTele == "") {

								$sql = "INSERT INTO telefone(telefone1, telefone2, idProfessor) VALUES ('$primaryTele', '$workTele', '$teste')";
								mysqli_query($conexao,$sql);
							} elseif ($workTele == "" && $secondaryTele != "") {

								$sql = "INSERT INTO telefone(telefone1, telefone3, idProfessor) VALUES ('$primaryTele', '$secondaryTele', '$teste')";
								mysqli_query($conexao,$sql);
							}
							 elseif ($workTele == "" && $secondaryTele == "") {
								$sql = "INSERT INTO telefone (telefone1, idProfessor) VALUES ('$primaryTele', '$teste')";
								mysqli_query($conexao,$sql);
							}
							

						header('Location: ../pages/dir-academico/cadastro-professor.php?sucesso=1');
					} else{
						echo "Estamos a ter difilcudades técnicas. Tente mais tarde.";
					}
				}
			...
			header('Location: ../pages/dir-academico/cadastro-professor.php?valor=1');
		}
